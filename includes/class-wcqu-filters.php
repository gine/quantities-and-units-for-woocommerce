	...
	*/
	public function input_set_all_values( $args, $product ) {
		...
		*/
		...
		elseif ( $on_sale and isset( $values['min_sale'] ) and $values['min_sale'] != ''  ) {
			$args['min_value'] = $values['min_sale'];
		}
		...
		} elseif ( $on_sale and isset( $values['max_sale'] ) and $values['max_sale'] != ''  ) {
			$args['max_value'] = $values['max_sale'];
		...
		// Set step value
		if ( $values['step'] != '' ) {
			//Fix step if near to Out of Stock
			if(strlen( $stock ) != 0 and $stock > 0 && $values['step'] > $stock){
				$args['step'] = $stock;
				$args['min_value'] 	 = $stock;
