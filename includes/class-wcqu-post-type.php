...
class WC_Quantities_and_Units_Quantity_Rule_Post_Type {
	...
	*/
	public function quantity_rule_init() {
		...
		$labels = array(
			'name'               => __('Quantity Rules','qau'),
			'singular_name'      => __('Quantity Rule','qau'),
			'add_new'            => __('Add New','qau'),
			'add_new_item'       => __('Add New Rule','qau'),
			'edit_item'          => __('Edit Rule','qau'),
			'new_item'           => __('New Rule','qau'),
			'all_items'          => __('All Rules','qau'),
			'view_item'          => __('View Rule','qau'),
			'search_items'       => __('Search Ruless','qau'),
			'not_found'          => __('No rules found','qau'),
			'not_found_in_trash' => __('No rules found in Trash','qau'),
			'parent_item_colon'  => '',
	...
	*/
	...
	public function quantity_rule_meta( $post ) {
		...
		$min_sale = get_post_meta( $post->ID, '_min_sale', true );
		...
		?>
			<div class="wpbo-meta">
				<label for="min"><?php echo __( "Minimum", 'qau' ); ?></label>
				<input type="number" name="min" id="min" value="<?php echo $min ?>" step="any" />

				<label for="max"><?php echo __( "Maximum", 'qau' ); ?></label>
				...
				<input type="number" name="min_sale" value="<?php echo $min_sale ?>" step="any" />

				<label for="_wpbo_maximum_sale"><?php echo __( "Sale Maximum", 'qau' ); ?></label>
				<input type="number" name="max_sale" value="<?php echo $max_sale ?>" step="any" />

				<label for="step"><?php echo __( "Step Value", 'qau' ); ?></label>
				<input type="number" step="any" name="step" id="step" value="<?php echo $step ?>" step="any" />

				<label for="step"><?php echo __( "Priority", 'qau' ); ?></label>
				<input type="number" name="priority" id="priority" value="<?php echo $priority ?>" />
			</div>
			<p><em><?php echo __( "*Note - the minimum value must be greater then or equal to the step value.", 'qau' ) ?></em><br />
			<em><?php echo __( "*Note - The rule with the lowest priority number will be used if multiple rules are applied to a single product.", 'qau' ); ?></em></p>
		<?php
	}
	...
	public function additional_info_notice_meta( $post ) {
		?>
			<div style="text-align: center">
				<h3><?php echo __("Enjoy this plugin?",'qau') ?></h3>
				<a href="http://wordpress.org/support/view/plugin-reviews/quantities-and-units-for-woocommerce" target="_blank"><?php echo __("Rate us on Wordpress.org!",'qau') ?></a>

				<h3><?php echo __("Need Support?",'qau') ?></h3>
				<a href="http://wordpress.org/support/plugin/quantities-and-units-for-woocommerce" target="_blank"><?php echo __("Visit our Support Forum",'qau') ?></a>
			</div>
		<?php
	}
	...
	*/
	public function save_quantity_rule_meta( $post_id ) {
		...
		if( isset( $_POST['min_sale'] ) ) {
			...
			update_post_meta( $post_id, '_min_sale', stripslashes( $min_sale ) );
		}
		...
		// Update Max Sale
		if ( isset( $_POST['max_sale'] ) ) {
			$max_sale = $_POST['max_sale'];

			// Max must be bigger then min
			if ( $max_sale != '' and isset( $min_sale ) and $min_sale != 0 ) {
				if ( $min_sale > $max_sale )
					$max_sale = $min_sale;

			} elseif ( $max_sale != '' and isset( $min ) and $min != 0 ){
				if ( $min > $max_sale ) {
					$max_sale = $min;
				}
			}

			update_post_meta( $post_id, '_max_sale', wcqu_validate_number( $max_sale ) );
		}
